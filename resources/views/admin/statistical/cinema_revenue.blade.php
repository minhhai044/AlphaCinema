...
@section('content')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                @if ($mostRewatchedMovies && $mostRewatchedMovies->isNotEmpty())
                    var rewatchCtx = document.getElementById('rewatchChart').getContext('2d');
                    var rewatchData = @json($mostRewatchedMovies);
                    if (rewatchData && rewatchData.length > 0) {
                        var labels = rewatchData.map(item => item.movie_name);
                        var rewatchValues = rewatchData.map(item => item.rewatch_count);
                        ...
                        new Chart(rewatchCtx, {
                            ...
                            options: {
                                ...
                                plugins: {
                                    title: {
                                        display: true,
                                        text: 'Phim Được Xem Lại Nhiều Nhất',
