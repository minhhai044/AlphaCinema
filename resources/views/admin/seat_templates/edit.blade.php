        ...
        <div class="col-lg-9 text-center">
            ...
            <div class="seat-map d-flex flex-column align-items-center">
                ...
                <div style="width: 80%;" class="card border-0 ">
                    ...
                    {{-- Nếu như chưa Publish  --}}

                    @if (!$seatTemplate->is_publish)
                        <table style="width: 95%;"
                            class="table-chart-chair table-none align-middle mx-auto text-center mb-5">
                            <tbody>
                                {{-- Duyện qua các vòng dựa trên số lượng max_row --}}
                                            ...
                                            @if ($seatType == 3)
                                                ...
                                                <td class="box-item border-1 {{ $rowClass }}"
                                                    ...
                                                    <div class="box-item-seat" data-type-seat-id="3">
                                                        <!-- 3 cho ghế đôi -->
                                                        <img src="{{ asset('images/seat/seat-2.png') }}" width="50%"
                                                            class='seat'>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="box-item-show">
                                                    <div class="seat-item">
                                                        @switch($seatType)
                                                            ...
                                                            @case(2)
                                                                <img src="{{ asset('images/seat/seat-1-free.png') }}"
                                                                    class='seat' width="70%">
                                                                <span class="seat-label">{{ chr(65 + $row) . $col + 1 }}</span>
                                                            @break
                                                        @endswitch
        ...
        <div class="col-lg-3">
            ...
            @if ($seatTemplate->is_publish)
                <form action="{{ route('admin.update_seat.seat_templates', $seatTemplate) }}" method="post">
                    ...
                    <a href="{{ route('admin.index.seat_templates') }}"><button type="button"
                            class="btn btn-warning">Quay lại</button></a>
                </form>
            @else
                <form id="seatForm" action="{{ route('admin.update_seat.seat_templates', $seatTemplate) }}"
                    method="post">
                    @csrf
                    @method('PUT')
                ...
                if (currentSrc.includes('seat-add.svg')) {
                    ...
                    if (typeSeatId === 3) {
                        ...
                        if (canMakeDoubleSeat(nextTd)) {
                            ...
                            img.attr('src', "{{ asset('images/seat/seat-2.png') }}").css('width',
                                '50%'); // set src cho img
                        } else if (canMakeDoubleSeat(previousTd)) { // nếu đằng trước td không còn thì sẽ quay lại đằng sau td
                            
                            // Chọn ghế đôi bên trái
                            previousTd.attr('colspan', 2); // set colSpan = 2 cho td trước td đang click
                            tdElement.hide(); // ẩn td bên trái
                            previousTd.find('img').attr('src', "{{ asset('images/seat/seat-2.png') }}").css('width','50%'); // set src cho img
                        } else {
                        ...
                        var seatSrc = (typeSeatId === 1) ? "{{ asset('images/seat/seat-1.png') }}" : "{{ asset('images/seat/seat-1.png') }}";
                ...
                $('.box-item-seat').each(function(index) {
                    ...
                    if (!img.attr('src').includes('seat-add.svg')) {
                        ...
                        seatStructure.push({
                            id: index + 1,
                            user_id: null,
                            price: type_seats.filter((item) => item.id == type_seat_id)[0]
                                .price,
